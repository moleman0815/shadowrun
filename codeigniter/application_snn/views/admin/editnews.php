<?php
	...
	foreach($categories as $c) {
		$selected = ($news[0]['category'] == $c['id']) ? 'selected="selected"' : '';
		$c_opt .= '<option value="'.$c['id'].'" '.$selected.'>'.$c['cat_name'].'</option>';
	}
?>
<script>
...
<div class="col-lg-8 admininterface">
		<fieldset>
		<legend class="newstitle">News editieren</legend>
		<br />
			<?php if($error): ?>
			...
			<?=form_open('/admin/editNews');?>
			<?=form_hidden('editNews', true);?>
			<?=form_hidden('id', $news[0]['id']);?>
			<div class="col-sm-6">
				<label class="control-label" for="title" style="width:150px">News Titel</label>				
				<?=form_input(array('id' => 'title', 'name' =>'title', "class" => "input-xlarge"), $news[0]['title']);?>
				<br />
			...
			<div class="col-sm-12">
				<br />	
				<?=form_submit(array('id'=>'submit', 'value' => 'News editieren', 'name' => 'submit', 'class' => 'btn btn-primary btn-sm'));?>
			</div>
			<?=form_close();?>			
	</fieldset>
	<br />
	<a href="/secure/snn/admin/newsVerwalten"><div class="newstitle"><i class="fa fa-arrow-circle-left"></i>&nbsp; zurück</div></a>
	<br />&nbsp;		
</div>