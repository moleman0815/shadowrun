...
#_debugDie($ganger);
	...
	function combatRound() {	
		...
		for($this->iniphase = max(array_keys($this->ini)); $this->iniphase > 0; $this->iniphase--) {
			$this->checkResult();
			
			if (in_array($this->iniphase, array_keys($this->ini))) {
				$this->fighterinround = $this->ini[$this->iniphase];
				$this->shootOut();
			}
