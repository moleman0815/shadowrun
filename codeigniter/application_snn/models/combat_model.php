...
// 			_debug($this->enemy[$target]);			
	...
	function evaluatePlayerDamage ($shots) {
		...
		if($this->status == 'running') {
			...
			if ($this->player['action'] == 'salve') {
				...
				array_push($this->combatlog, 'Fernkampf -> Angriff: <b>'.ucfirst($this->player['name']).'</b> feuert einen automatischen Feuersto� und steigert seinen Schaden von <b>'.$dmgBefore.'</b> auf <b>'.$this->player['weapon_damage'].'</b>.<br />');
			}
			if (count($shots) > 2) {
				$dmgBefore = $this->player['weapon_damage'];
	...
	function writeResults() {
		...
		if ($this->status == 'success') {
			...
			$loss = '0';
			$money = $this->player['money']+cash;
			array_push($this->combatlog, 'Du wurdest ohnm&auml;chtig, aber deine Gegner sind tot. Der Run hat dir '.$cash.' &yen; gebracht. Gl�ck gehabt!<br />');
		} else {
			$result = 'Lost';
			$cash = '0';
